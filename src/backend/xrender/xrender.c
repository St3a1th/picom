...
static void compose(backend_t *base, struct managed_win *w, void *img_data, int dst_x,
	...
	if (cr == 0) {
		...
		// log_warn("f(%d, %d) imge(%d %d) imgf(%d %d) sdw(%d %d) dst(%d %d) s:%d
		// b:%d", fullwid, fullhei, img->ewidth, img->eheight, img->width,
		// img->height, w->shadow_width, w->shadow_height, dst_x, dst_y,
		// w->shadow, w->g.border_width);
