...
#ifdef CONFIG_OPENGL
	case BKEND_GLX:
		glx_round_corners_dst(ps, (w ? w->paint.ptex : ps->root_tile_paint.ptex),
		                      shader_idx, x, y, wid, hei, (float)ps->psglx->z - 0.5f,
		                      cr, reg_paint, &w->glx_round_cache);
		break;
#endif
	default: assert(0);
	}
...
#undef COMP_BDR
	...
	if (argb && w->corner_radius > 0) {
	        ...
	        win_round_corners(ps, w, 0, (float)w->corner_radius, ps->tgt_buffer.pict,
	reg_paint);
	        //glDisable(GL_BLEND);
	}*/
}
