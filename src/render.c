...
win_round_corners(session_t *ps, struct managed_win *w, int shader_idx, float cr,
	...
	case BKEND_GLX:
		if (shader_idx == 1) {
			glx_round_corners_dst1(
			    ps, (w ? w->paint.ptex : ps->root_tile_paint.ptex),
			    shader_idx, x, y, wid, hei, (float)ps->psglx->z - 0.5f, cr,
...
void paint_all(session_t *ps, struct managed_win *t, bool ignore_damage) {
	...
	for (auto w = t; w; w = w->prev_trans) {
		...
		//	win_round_corners(ps, w, 0, (float)w->corner_radius,
