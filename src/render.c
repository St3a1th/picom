...
void render(session_t *ps, const struct managed_win *w, int x, int y, int dx, int dy,
            int wid, int hei, int fullwid, int fullhei, double opacity, bool argb,
 ...
 */
static inline void
win_round_corners(session_t *ps, struct managed_win *w, float cr,
...
#undef COMP_BDR
	...
	if (w->corner_radius > 0) {
		...
		win_round_corners(ps, w, (float)w->corner_radius, ps->tgt_buffer.pict,
		                  reg_paint);
	}
}
	...
	case BKEND_XR_GLX_HYBRID: {
		...
		if (cr) {
			...
			xcb_render_trapezoids(
			    ...
			    solid_picture(ps->c, ps->root, false, 1, 0, 0, 0), td,
